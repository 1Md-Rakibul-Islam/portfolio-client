...
const Blogs = () => {
  ...
  const {
    ...
    queryKey: ["blogs"],
    queryFn: async () => {
      const { data } = await axios.get("https://portfolio-rakibul-islam.vercel.app/blogs");
  ...
  return (
    <section className="min-h-screen my-10 md:mx-20 mx-5">
      <h1 className="text-center mt-40 text-4xl">{blogs?.length > 0 ? "" : "Comming Soon..."}</h1>
			<h2 className="text-3xl text-white mb-16 text-center font-bold">Wellcome to my blogs page</h2>
